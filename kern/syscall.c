...
sys_exofork(void)
{
    ...
    struct Env* e;
    int errno;
    errno = env_alloc(&e, curenv->env_id);
    if (errno != 0)
        return errno;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
    ...
    // check va
    if ((uint32_t) srcva >= UTOP || ROUNDDOWN(srcva, PGSIZE) != srcva) return -E_INVAL;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
    ...
    struct Env* dst_env;
    // args sanity check
    if ((errno = envid2env(envid, &dst_env, 0)) != 0)
        return errno;
    if (!dst_env->env_ipc_recving) return -E_IPC_NOT_RECV;
    // not sure
    assert((uint32_t) srcva <= UTOP);
    dst_env->env_ipc_perm = 0;
    if ((uint32_t) srcva % PGSIZE != 0) return -E_INVAL;
    if ((uint32_t) srcva != UTOP)
    {
        // check perm
        if ((perm & (PTE_P | PTE_U)) != (PTE_U | PTE_P)
            || (perm & ~PTE_SYSCALL))
            return -E_INVAL;
        pte_t *pte;
        struct PageInfo* pp = page_lookup(curenv->env_pgdir, srcva, &pte);

        if (!pp || (!(*pte & PTE_W) && (perm & PTE_W)))
            return -E_INVAL;
        if ((errno = page_insert(dst_env->env_pgdir, pp, dst_env->env_ipc_dstva, perm)) != 0)
            return errno;
...
sys_ipc_recv(void *dstva)
{
    ...
    if (dstva)
    {
        if ((uint32_t) dstva > UTOP)
            panic("sys_ipc_recv: trying to access kernel memory space\n");
